...
function transformForDashboard() {
  ...
  data.forEach(row => {
    ...
    // (linkedin_msg_replied_count). A lead Aimfox never mapped still gets DMs
    // and replies, recorded only by the latter — reading Aimfox alone silently
    ...
    // Fallback for replies the native LinkedIn sync caught but Aimfox did not:
    // those have no linkedin_response_aimfox_at, which would leave the reply
